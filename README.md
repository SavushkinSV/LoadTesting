...
Все объекты, которые явно или неявно создаются Java-приложением, размещаются в куче.  Cборщики мусора используют разные подходы к организации памяти и ее очистке, но их объединяет общая черта — все они опираются на слабую гипотезу о поколениях. В общем виде, гипотеза о поколениях гласит, что вероятность смерти как функция от возраста снижается очень быстро.

![Распределение времени жизни обхектов](/images/image_11.png)
...
Вот тут и возникает идея разделения объектов на младшее поколение (__young generation__) и старшее поколение (__old generation__). В соответствии с этим разделением и процессы сборки мусора разделяются на малую сборку (__minor GC__), затрагивающую только младшее поколение, и полную сборку (__full GC__), которая может затрагивать оба поколения. Малые сборки выполняются достаточно часто и удаляют основную часть мертвых объектов. Полные сборки выполняются тогда, когда текущий объем выделенной программе памяти близок к исчерпанию и малой сборкой уже не обойтись.
...
__Epsilon GC__ — разработан для случаев, когда сборка мусора вообще не нужна. \
__ZGC__— пытается удерживать паузы на субмиллисекундном уровне, даже при работе с очень большими кучами. \
__Shenandoah GC__ — еще один сборщик, нацеленный на ультракороткие паузы независимо от размера кучи.

В большинстве систем управления памятью с автоматическим сборщиком мусора (GC) можно выделить три основные стадии работы:

__Маркировка (Marking)__ — на этом этапе сборщик мусора определяет, какие объекты в памяти всё ещё используются программой, а какие стали недоступными и могут быть удалены. Процесс маркировки начинается с корневых объектов (например, локальных переменных, статических полей и т. д.) и рекурсивно проходит по всем ссылкам, отмечая доступные объекты.

__Очистка (Sweep или Sweeping)__ — после того как все доступные объекты отмечены, сборщик мусора проходит по области памяти и освобождает пространство, занятое недоступными объектами, которые не были отмечены в процессе маркировки. На этом этапе память, занятая удаляемыми объектами, помечается как свободная для последующего использования.

__Дефрагментация (Compact или Compacting)__ — на этой стадии (которая может быть необязательной в некоторых реализациях GC) происходит перемещение оставшихся объектов в памяти для устранения фрагментации. Это позволяет оптимизировать использование памяти и упростить последующие операции выделения памяти для новых объектов.
...
![Разделение кучи на области при использовании Serial GC](/images/image_12.png)

Объект начинает свою жизнь в регионе Eden. Но со временем может оказаться так, что места для вновь создаваемого объекта в Eden нет, в таких случаях запускается малая сборка мусора.
...
![Работа малой сборки мусора Serial GC](/images/image_13.png)
...
![Работа малой сборки мусора Serial GC](/images/image_14.png)
...
![Работа малой сборки мусора Serial GC](/images/image_15.png)
...
![Работа малой сборки мусора Serial GC](/images/image_16.png)
...
![Работа малой сборки мусора Serial GC](/images/image_17.png)
